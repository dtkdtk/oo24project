...
- `(subtop) (top) dupsub` - дублировать значение (`subtop`) на ПРЕД-верхушке стека.
- `(value) (...skipped...) (pos) dupat` - дублировать значение на указанной позиции относительно верхушки.
> Позиция всегда отрицательная; -1 это пред-верхушка, -2 это пред-пред-верхушка и т.д.
> Примечание: Аргумент `(pos)` пропускается (не является верхушкой стека).
...
- `(warncode) disable_warning` - отключает сообщение о предупреждении с указанным кодом.
> Если уже отключено - ничего не происходит.
> [НЕ РЕКОМЕНДУЕТСЯ]: Используйте `*` чтобы отключить ВСЕ предупреждения.
- `(warncode) enable_warning` - включает обратно сообщение о предупреждении с указанным кодом.
> Если и так включено - ничего не происходит.
> Используйте `*` чтобы включить ВСЕ предупреждения.
> По умолчанию, включены только "критические" предупреждения.
...
- `(a) (b) [/]` - поделить число `a` на число `b`. Результатом может быть число с плавающей точкой.
